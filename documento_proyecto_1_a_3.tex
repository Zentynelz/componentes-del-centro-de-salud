...
Para este proyecto usamos las siguientes herramientas:

\begin{itemize}
    \item \textbf{Sistema Operativo -- Windows 10/11:} Usamos Windows porque es lo que hay en la mayoría de centros de salud y todo el mundo sabe usarlo.

    \item \textbf{Motor de Base de Datos -- MySQL:} MySQL porque es el que más se usa, es gratis y funciona bien para lo que necesitamos. Además soporta vistas, triggers y todo eso que necesitamos.

    \item \textbf{Herramientas de Desarrollo:}
    \begin{itemize}
        \item \textit{MySQL Workbench:} Para dise\~nar y administrar la base de datos gr\'aficamente, ejecutar consultas y ver los resultados de forma visual.
        \item \textit{Visual Studio Code:} Como editor de c\'odigo principal para desarrollar la aplicaci\'on en Python y gestionar el proyecto.
        \item \textit{Python 3.x con Streamlit:} Para construir la interfaz gr\'afica de usuario de forma r\'apida y sencilla. Streamlit permite crear aplicaciones web interactivas escribiendo solo Python.
    \end{itemize}
\end{itemize}
...
\item \textbf{Validaciones de integridad avanzadas.} Adem\'as de las reglas generales del centro de salud, la base de datos debe impedir autom\'aticamente registros inconsistentes. Por esta raz\'on, se implementan restricciones y disparadores (triggers) que validan que los horarios de un mismo m\'edico no se traslapen, que un empleado no tenga periodos de vacaciones cruzados, que solo los m\'edicos de tipo Sustituto puedan registrar periodos de sustituci\'on y que los diagn\'osticos no tengan fechas futuras.
...
\item \textbf{Asignaci\'on segura de pacientes.} El sistema debe impedir que se asignen nuevos pacientes a m\'edicos que no se encuentren disponibles en el momento del registro, incluyendo m\'edicos en vacaciones y m\'edicos sustitutos inactivos.
...
\begin{itemize}
    ...
    \item \textbf{Entidades d\'ebiles:} Algunas tablas aparecen con doble recuadro porque dependen de otra tabla para existir. Por ejemplo, un horario no tiene sentido si no est\'a asociado a un m\'edico, y un diagn\'ostico siempre es de alg\'un paciente.
